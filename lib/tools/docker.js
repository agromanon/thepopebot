 ...
 * @param {string} [options.volume] - Docker volume name
 * @param {string} [options.chatContext] - JSON chat context for the container
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runCodeWorkspaceContainer({ containerName, repo, branch, codingAgent, featureBranch, volume, workspaceId, chatContext }) {
  ...
  const hostConfig = {};
  const vol = volume || (workspaceId ? volumeName(workspaceId) : null);
  if (vol) {
    hostConfig.Binds = [`${vol}:/home/coding-agent`];
  }
...
function buildAgentAuthEnv(agent) {
  ...
  if (agent === 'claude-code') {
    ...
    if (builtinKeyMap[provider]) {
      ...
      // Pass ANTHROPIC_BASE_URL for custom Anthropic-compatible endpoints
      if (provider === 'anthropic') {
        const baseURL = getConfig('ANTHROPIC_BASE_URL');
        if (baseURL) env.push(`ANTHROPIC_BASE_URL=${baseURL}`);
      }
    } else {
      // Custom provider
 ...
 * @param {string} [options.volume] - Docker volume name (or null for ephemeral)
 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runHeadlessCodeContainer({ containerName, repo, branch, featureBranch, volume, taskPrompt, mode = 'plan', codingAgent, systemPrompt, continueSession }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
  ...
  const hostConfig = {};
  if (volume) {
    hostConfig.Binds = [`${volume}:/home/coding-agent`];
  }
